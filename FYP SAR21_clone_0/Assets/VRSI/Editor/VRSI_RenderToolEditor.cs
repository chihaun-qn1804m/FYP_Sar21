...
[CustomEditor(typeof(VRSI_RenderTool))]
public class VRSI_RenderToolEditor : Editor
{
    private VRSI_RenderTool myTarget;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!myTarget.getInVRMode())
        {
            ...
            boxStyle.normal.textColor = new Color(0.95f, 0.8f, 0);
            ...
            GUILayout.FlexibleSpace();
            myTarget.vrsiResolution = (VRSI_RenderTool.VRSI_Resolution)EditorGUILayout.EnumPopup("", myTarget.vrsiResolution, GUILayout.Width(150));
            GUILayout.EndHorizontal();

            if (myTarget.vrsiResolution == VRSI_RenderTool.VRSI_Resolution.VeryLow_256)
            {
                myTarget.vrsiRenderSize = 128;
            }
            else if (myTarget.vrsiResolution == VRSI_RenderTool.VRSI_Resolution.Low_512)
            {
                myTarget.vrsiRenderSize = 256;
            }
            else if (myTarget.vrsiResolution == VRSI_RenderTool.VRSI_Resolution.Medium_1024)
            {
                myTarget.vrsiRenderSize = 512;
            }
            else if (myTarget.vrsiResolution == VRSI_RenderTool.VRSI_Resolution.High_2048)
            {
                myTarget.vrsiRenderSize = 1024;
            }
            else if (myTarget.vrsiResolution == VRSI_RenderTool.VRSI_Resolution.VeryHigh_4096)
            {
            ...
            GUILayout.FlexibleSpace();
            myTarget.vrsiResolutionNormal = (VRSI_RenderTool.VRSI_Resolution)EditorGUILayout.EnumPopup("", myTarget.vrsiResolutionNormal, GUILayout.Width(150));
            GUILayout.EndHorizontal();

            if (myTarget.vrsiResolutionNormal == VRSI_RenderTool.VRSI_Resolution.VeryLow_256)
            {
                myTarget.vrsiRenderSizeNormal = 256;
            }
            else if (myTarget.vrsiResolutionNormal == VRSI_RenderTool.VRSI_Resolution.Low_512)
            {
                myTarget.vrsiRenderSizeNormal = 512;
            }
            else if (myTarget.vrsiResolutionNormal == VRSI_RenderTool.VRSI_Resolution.Medium_1024)
            {
                myTarget.vrsiRenderSizeNormal = 1024;
            }
            else if (myTarget.vrsiResolutionNormal == VRSI_RenderTool.VRSI_Resolution.High_2048)
            {
                myTarget.vrsiRenderSizeNormal = 2048;
            }
            else if (myTarget.vrsiResolutionNormal == VRSI_RenderTool.VRSI_Resolution.VeryHigh_4096)
            {
            ...
            GUI.backgroundColor = new Color(0.9f, 0.9f, 0.2f);
            GUILayout.Box("Select The Versions To Generate", GUILayout.ExpandWidth(true));
            GUI.backgroundColor = tmpbgcol;


            GUI.color = Color.cyan;
            myTarget.Create_For_VR = GUILayout.Toggle(myTarget.Create_For_VR, "Create For VR (Standard VRSI)", myToggleStyle);
            GUILayout.Space(10);
            myTarget.Create_For_PC_MOBILE = GUILayout.Toggle(myTarget.Create_For_PC_MOBILE, "Create For PC-MOBILE (Hight-Quality Billboards)", myToggleStyle);
            GUI.color = tmpGuiCol;
        ...
        if (Application.isPlaying)
        {
            if (!myTarget.getInVRMode())
            {
                ...
                {
                    if (!myTarget.Create_For_VR && !myTarget.Create_For_PC_MOBILE)
                    {
                        EditorUtility.DisplayDialog("Select A Version!", "Please Select At Least One Version To Generate", "Ok");
                    }

                    if (myTarget.Create_For_VR)
                    {
                        myTarget.CreateVRSI();
                    }
                    else
                    {
                        if (myTarget.Create_For_PC_MOBILE)
                        {
                            myTarget.CreateVRSI_PC_MOBILE();
                        }
                    }
    ...
    protected virtual void Awake()
    {
        ...
        myTarget = (VRSI_RenderTool)target;

        logoTex = Resources.Load<Texture>("Textures/VRSI_LOGO");
...
#endif
        backcol2.normal.background = MakeTex(1, 1, new Color(0, 0.2f, 0.4f, 1));
    }
