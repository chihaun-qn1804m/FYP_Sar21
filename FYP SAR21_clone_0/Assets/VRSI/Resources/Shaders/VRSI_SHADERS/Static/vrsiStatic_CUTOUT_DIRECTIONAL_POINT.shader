...
Shader "VRSI/Static/vrsiStatic_CUTOUT_DIRECTIONAL_POINT"
{
	Properties
	{
...
#define RAD2DEG 57.2957795131
	...
	vertexOutput vert(vertexInput input)
	{
		...
		if (input.tex.y > _OffsetY)
		{
			...
			//float noise2 = tex2Dlod(_NoiseTex, float4(float2(_Time.y*_WindTrunkSpeed*0.1+ (output.noiseVal.x + output.noiseVal.y), 0)*_ScaleX*0.25, 0, 0)).r;
			//noise2 += tex2Dlod(_NoiseTex, float4(float2(_Time.y*_WindTrunkSpeed*2 + (output.noiseVal.x + output.noiseVal.y+100), 0)*_ScaleX*3, 0, 0)).r;
			//input.vertex.x += (noise2-0.2)*_WindTrunkStrength*0.04;

			//input.vertex.x += sin(_Time.y * 4.3*_WindTrunkSpeed + (output.noiseVal.x + output.noiseVal.y) * 100)*0.0085*_WindTrunkStrength;
		}
		else
		{
