...
Shader "VRSI/Static/Specular/vrsiStatic_CUTOUT_DIRECTIONAL_SPECULAR"
{
	Properties
	{
	...
	vertexOutput vert(vertexInput input)
	{
		...
		output.viewDirection = normalize(float3(float4(_WorldSpaceCameraPos.xyz, 1.0) - mul(unity_ObjectToWorld, input.vertex).xyz));
	...
	float4 frag(vertexOutput input) : COLOR
	{
		...
		float3 specularReflection = _SpecularColor.rgb  * pow(max(0.0, dot(reflect(-lightDirection, norm), input.viewDirection)), _Shininess);
		//===		
